...
Updated 2026-09-22 (declared architecture variant): the owner authorized the architectural variant
that F3 needed, and it is implemented in `global_policy.py`: `GlobalMacroPolicy(variant,
main_architecture)` with `MAIN_ARCHITECTURES` `("resnet18", "compact")`, an unknown architecture
refused at construction, `main_architecture` written into every new checkpoint's metadata, and
`architecture_from_metadata` reading it - where a metadata without the key is read as resnet18, which
is the compatibility rule that keeps every frozen checkpoint on disk loadable without rewriting it and
leaves `human_ifo.py` and `human_inverse.py` unchanged. The pipeline then connects end to end on the
frozen dev split: the frozen teacher measures a median of 480.1 ms at intent macro-F1 0.8891 with
11,383,694 parameters, an untrained compact student measures a median of 5.59 ms at intent macro-F1
0.0039 with 482,606 parameters, and the gate returns `passed=false` with the single reason
`intent_macro_f1_regressed` - 86 times faster and 95.8 percent smaller is still rejected because the
behaviour collapsed, which is exactly the declared rule that a latency win and a parameter win are not
substitutes. `make check` passes with Ruff, strict mypy and 676 tests. No candidate was trained, no
batch run, no checkpoint promoted, no device used and no source changed; the frozen Global Agent
holdout stays the acceptance for any candidate that changes control, and the F1 chain, the cold-start
limit, the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (F3 pilot, negative): the owner authorized the distillation pilot, and
`distill_global_main` (CLI `global-agent-distill`) was added to the allowlisted `global_policy.py`: it
copies every non-`main` weight from the frozen teacher, freezes it, distils the teacher's logits on the
declared train split within a bounded step budget, verifies afterwards that no frozen parameter moved,
and writes a candidate that is explicitly not promoted. Two bounded configurations were run on the
frozen pilot (2,016 train, 510 dev windows) and both were rejected by the unchanged gate: at 200 steps
and lr 0.01 the dev intent macro-F1 reached 0.3800 while the loss rose from 57.9 to 62.3, and at one
epoch of 252 steps and lr 3e-4 it reached only 0.0683 with an unstable trace (min 26.7, max 147). Both
candidates were about 99 percent faster (5.5 ms against about 511 ms per batch) and 95.8 percent
smaller, and both failed with the single reason `intent_macro_f1_regressed`, against the frozen
teacher's 0.8891. The result is therefore a working pipeline and a negative pilot, and per the pack no
data, time or parameter expansion follows a result without a verified gain: the compact main is cheap
enough that the latency half has headroom and the whole question is behaviour recovery, while the
current MSE-on-logits objective is not a well-behaved signal (the dev result is not monotone in the
budget). Nothing was promoted, no control changed, no device was used, and the frozen 20-seed holdout
remains the acceptance for any candidate that changes control. `make check` passes with Ruff, strict
mypy and 678 tests. The F1 chain, the cold-start limit, the mask scope and the bound pass are
unchanged.
Updated 2026-09-22 (F3, gentler backbone): the owner asked for a gentler compression than a different
network, so a `resnet18_shallow` variant was declared - the same resnet18 family, shapes, widths and
512-wide output, with the second block dropped from each layer, keeping 55.1 percent fewer parameters
(main 11,168,832 to 4,898,112). This answers the behaviour question: the same-family shallow `main`
moved dev intent macro-F1 from 0.0039 untrained to 0.8526 at 252 steps, then to 0.8772 at 900 steps,
and it clears the latency half comfortably at a 43.3 percent median and 42.4 percent p95 reduction
against the declared 25 percent. The unchanged gate still returns `passed=false`, now for a
behaviour drop of 0.0119 against the declared maximum of 0.0 - so the remaining question is the
acceptance policy, not an engineering unknown. The gate's zero tolerance was not loosened, nothing was
promoted, no control changed and no device was used; a fitted student could pass once the owner
decides whether a small declared behaviour tolerance is acceptable, or exact parity is trained for
first. `make check` passes with Ruff, strict mypy and 678 tests. The F1 chain, the cold-start limit,
the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (F3, a candidate passes): rather than relax the zero tolerance to admit the
0.0119-drop candidate, the same shallow family was fitted further across seeds, and seed 1 reaches dev
parity and **passes the unchanged gate on its own merit**: dev intent macro-F1 0.8891 against the
teacher's 0.8891 (+0.00006) with a 42.8 percent median and 41.8 percent p95 latency reduction, on a
`resnet18_shallow` main of 5,112,974 parameters against 11,383,694. Seeds 0 and 2 at smaller budgets
were rejected (-0.0119 and -0.0038), so the same declared threshold has now both rejected and passed
the same compression, which is the evidence that the F2 feedback discriminates. The zero-intent-drop
tolerance was reviewed with the owner and left unchanged; no tolerance was loosened. The result is a
dev-split signal and the split was read many times while iterating on seeds, so it is not a final
acceptance; the frozen 20-seed holdout stays unopened and remains the acceptance for any candidate
that changes control. Nothing was promoted, the candidate is not wired into the runtime, and
`promotion_allowed` stays false on every artifact. `make check` passes with Ruff, strict mypy and 678
tests; the F1 chain, the cold-start limit, the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (F3, holdout rejects the candidate): the passing dev candidate was run against the
frozen 20-seed simulator holdout, which is fully offline. The candidate was entered as the `dagger`
slot and the frozen DAgger baseline as the `adapted` slot because the function only names roles and the
selection rule keeps the frozen model when the candidate is not better. The baseline retains selection
(`selected_model=adapted`): non-timeout terminals 18 against 14 for the candidate, mean tower damage
12.0 against 11.55, stuck-time ratio 0.0499 against 0.0614 and fallback rate 0.0563 against 0.0689,
with both eligible and no safety or invalid-action violations. A paired read shows the two agree on 14
of 20 seeds, the candidate wins only seed 4005, and the baseline wins 5 seeds the candidate draws
(4004, 4006, 4007, 4009, 4015) - and on all five the candidate reaches the same tower damage and ends
in `draw_tick_limit` while the baseline converts it to a crystal win in 63-96 ticks. So the gap is
pacing at the decision boundary rather than lost capability, and its fallback rate is higher on those
seeds. An unpaired exact test on 14 against 18 gives p = 0.235, so the value is the paired structure
and the shared tick-limit outcome. **Dev parity did not survive acceptance**: the F2 feedback passed a
candidate on dev and the frozen holdout then rejected it, which is the gate doing its job. Nothing was
promoted, no threshold was changed, no control moved, no device was used, and the frozen holdout stays
the acceptance for anything that changes control. `make check` passes with Ruff, strict mypy and 678
tests; the F1 chain, the cold-start limit, the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (F3, cause of the holdout rejection measured): before changing anything, the
failure was diagnosed by replaying the holdout path by hand, and it is not what a first guess
suggests. Two measurements pin it: (1) the frozen rule teacher itself draws when it plays seed 4006
alone, issuing `DISENGAGE/OWN_BASE` then `RECALL/OWN_BASE` from the tick the red tower falls, so there
is no teacher behaviour that converts the win; and (2) the baseline's holdout wins come from the
mixed-authority path rather than from winning alone - at the holdout's authority 1.0 the candidate wins
seed 4000 and draws 4004/4005/4006/4007/4009/4015, while at authority 0.25 it wins 4004/4006/4009 and
draws the rest, because the teacher drives three ticks in four and the student's
`PUSH_STRUCTURE/ENEMY_BASE` proposals appear often enough to push the crystal. When the student does
reach the crystal phase its post-tower proposals are already correct (seed 4000 shows
`PUSH_STRUCTURE/ENEMY_BASE` in all 14 crystal ticks), so it knows what to do; it does not reliably reach
...
dataset, which contains only states the rule teacher visits; since the teacher never contests the
crystal alone, the conversion states were missing from training entirely. `distill_global_main` gained
an optional `auxiliary_windows` argument and the CLI an `--auxiliary-windows` flag that mixes those
already-persisted windows into the same objective as cross-entropy on the teacher's own recorded
...
the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (first candidate Shadow smoke, device): the candidate read-only Shadow path was
exercised once on hardware, 60 seconds, and passed. Foreground was `com.tencent.tmgp.sgame` at
1600x720 rotation 1, scrcpy 1.25 fed the `global-agent-shadow` v4l2loopback at `/dev/video10`
(720x1600 YU12, orientation locked), and the run used the unchanged observation ROIs (`876adf76...`)
with the candidate checkpoint `d9f18bb9...`. Result: `status=PASSED`, `candidate_run=true`, schema
`hok-agent-global-shadow-candidate-v1.1`, `cycle_coverage` 1.0 over 114 scheduled and 114 completed
cycles at 1.90 Hz, `stale_frame_count` 0, `hard_stop_cycles` 0, `safety_violations` 0, and the
zero-control invariants held exactly - `control_output` false, `device_input_allowed` false,
`input_commands_sent` 0. Latency was p50 39.44 / p95 43.20 ms inference and p50 46.96 / p95 51.81 ms
end to end against the frozen 500 ms gate. The model proposed `DISENGAGE/OWN_BASE` for all 114 cycles
with zero abstentions and zero macro transitions, which is consistent with the placement being on the
own-base side and is a statement about this single screen, not a behavioural result. This is one
smoke, not an acceptance or a comparison: it shows the isolated candidate path runs on device and
stays read-only, and it does not compare against the promoted model or claim any speedup. The
promoted Shadow contract is untouched, and the F1 chain, the F2 feedback, the F3 result, the
cold-start limit, the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (candidate Shadow formal comparison): the owner asked for the formal candidate
comparison, and the fair way to do it is not two live runs - two live Shadow sessions see different
screens and cannot be compared. Shadow gained an opt-in `--persist-windows` archive (the exact
sampled windows it decided on) and a new `global-agent-shadow-compare` command that scores both
checkpoints on one persisted archive, so the comparison is paired by construction. A 600-second
candidate Shadow attempt on device was stopped at 438 seconds and 817 cycles by the guard, correctly,
because a Huawei resolver sheet took the foreground mid-run (`guard_or_capture_error`); the guard
refusing to keep deciding off an unauthorized foreground is the protection working, and the 817 cycles
captured before that are the comparison evidence. The comparison is the line's strongest result and it
is also where the line ends: on the same 817 windows the candidate and the frozen baseline agree on
**99.63 percent of full decisions** (99.76 percent on intent, 99.88 percent on zone) at a **45.9
percent median latency reduction** (48.61 to 26.27 ms) with 55.1 percent fewer parameters, and the
candidate is if anything slightly more confident (0.989 against 0.944). So the candidate does not
merely fail to lose the holdout - it reproduces the frozen policy's decisions almost exactly on real
device frames while being cheaper, which is the whole point of the compression line. Scope, stated
plainly: this is **non-inferiority and cost, not a win**. 99.63 percent agreement is not exact
equivalence (about three windows in 817 differ), it is not a game-outcome measurement, and the
candidate remains unpromoted with `promotion_allowed` false and zero control (`control_output` false,
`device_input_allowed` false, `input_commands_sent` 0). Both the 60-second smoke and this comparison
are read-only. Everything is now delivered and pushed: F1, F2, F3 and the candidate Shadow line.
`make check` passes with Ruff, strict mypy and 688 tests. The promoted Shadow contract is untouched,
and the cold-start limit, the mask scope and the bound pass are unchanged.
Updated 2026-09-22 (B interface decision): with the compression line closed and the main objective
blocked on a truth source, the only remaining work that is not invented activity is to make the
owner's choice concrete, so the interface decision for a source is recorded in
[`docs/execution/c783402-next/06_truth_source_interface_decision.md`](docs/execution/c783402-next/06_truth_source_interface_decision.md).
It states what any usable source must supply - hero position in a fixed frame (F1) and/or the match
terminal and its ending tick (F2), with structure state (F3) and life/death moments (F4) as sharpeners
- and sets out the three candidate shapes: an app-side export, the owner's app source tree, or a
controlled ground-truth capture. It also bounds the gain honestly: even a perfect source would prove
whether the cue-localised position is accurate and whether a terminal was real, but it would not prove
the policy is good or yield a match win rate. Nothing is implemented and no adapter is written until the
owner names a source, because a stub importer or an assumed schema is exactly the fabrication this
project forbids. The decision requested from the owner is one line: source kind, what it is, which of
F1-F4 it provides, and how it is obtained.
Updated 2026-09-22 (B acceptance declared): the truth-source acceptance was recorded without
building anything, so the owner's one-line source declaration has a fixed target. It lives at
`game_rules/truth_source_acceptance_v1.json` with `status: PROPOSED_NOT_IMPLEMENTED`, reuses the
existing R0 gates verbatim (its duplication guard is byte-equal to the R0 contract's), states what a
source must supply (F1 hero position and F2 match terminal at minimum, with F3 structure state and F4
life-and-death as sharpeners), enforces binding by timestamp with refusal of any field the declared
schema does not name, and records the honest bound: a source would settle whether the cue-localized
position is accurate, whether a recorded terminal was real, and whether the cold start is genuinely
impassable - but not whether the policy is good and not a match win rate. `implementation_gate`
forbids a stub importer, an assumed schema and any claim the condition is met before the owner names
