...
NEXT_ACTION: decide whether to force the three consecutive misses that would exercise the bounded recovery sweep, or drop the sweep as unneeded, since it is implemented and unit-covered but its trigger was not reachable in the attempt recorded here; the delivered deterministic chain otherwise stays frozen
...
NEXT_DECISION: only an external independent reference or a scene with a genuinely discrete, independently measurable state reopens R; otherwise the deterministic chain is the delivered result and the frozen R1 contract stays unexecuted
```
