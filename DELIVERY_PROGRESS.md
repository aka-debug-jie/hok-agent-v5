...
NEXT_ACTION: run the declared detour against the cold-start limit on device, stage 1 first, because the capability is implemented and validated offline but has never moved the hero: place the hero at the recorded failing start `(78.4, 55.4)` with the tight placement, then run route B v18 (`f6ae612becc7`) and check that the trigger fires at the recorded stall cell `14:12`, that the plan is the declared `NE NW`, and that the episode either arrives or ends as `detour_exhausted` rather than a timeout. The route B v15 pass scope and the death-confirmation policy are unchanged by this, the cold-start limit still stands until the run passes, and the mask's isolated contribution remains unmeasured
...
NEW_BYTES: 1,677,642,749 used by the active-probe and goal-navigation lineages to date; the 268,435,456 question cap is exceeded and the owner has stated there is no budget limit (this total adds the 120,626,453 of the bound rebind - the 3,415,769 tight placement and the 8,478,802, 25,963,286 and 82,768,596 stage-1, stage-3 and stage-10 batches - to the previous 1,557,016,296, which added the 14,424,993 of the fresh `route-b-rebind2-stage1` run, which exercised the death-confirmation policy on device, to the 1,542,591,303 before that, which added the 3,634,071 of the death-box negative check - eighty-three read-only live top-strip frames, the owner death strip and their summary, under `audit/hierarchical-movement-mvp/death-box-negatives-v1/` - to the 1,538,957,232 before that, which added the 47,506,477 of the 2026-09-22 rebind - the 8,924,418 `route-b-rebind-stage1` batch, the 35,444,272 `route-b-rebind-stage3` batch and the 3,137,787 reposition - to the 1,491,450,755 before that, which itself added the 222,454,107 of the five route B v15 device runs, the 4,248 of the probe re-analysis output, the 29,551,263 of the two probe sessions and their analysis, the 170,099 of the placement and pre-flight images, the 1,828,915 of the preserved death reference, the 85,962,294 of the two probe calibration runs and their analyses, the 2,270,045 of the two repositions and the cold-start batch, the 88,363,295 of the v16 and v17 cold-start attempts and their repositions, and the route B review's 4,177,652 completed form to the 1,060,808,197 before that)
...
NEXT_DECISION: accept the pass as now bound to the current code (14 of 14 from a 1.5 px placement, contract `ba46e2b2`, death policy live, ROIs `876adf7626a1`) and choose the next limit rather than another rebind: declared detour planning for the cold-start limit, which is drafted but not implemented, or a controlled traversability probe for the mask's unmeasured isolated contribution, or a captured frame of the offending non-death banner to replace the counted rejections with a picture; do not re-run the admission, do not start the life-state ROI, do not reuse the mobile banner geometry on native footage, and do not retune the frozen detector
```
