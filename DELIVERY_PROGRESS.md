...
NEXT_ACTION: re-run the route B v15 staged admission against the unchanged contract `ba46e2b2` with the death-confirmation policy now live, stage 1 first, because the 2026-09-22 rebind failed on a death-box false positive and that cause is now fixed rather than worked around: the box and its 2000/80 thresholds are unchanged, and the stop now additionally requires the banner to hold for `confirmation_steps` 2 consecutive observations and the localised hero to travel no more than `maximum_travel_pixels` 1.0 over `stationary_window_steps` 2, which refuses the recorded stop step on both halves (streak one, travel 4.24 px) while the owner death reference still reads death-visible. The rebind's failure stands as recorded until the fresh run passes, the fourteen-of-fourteen pass keeps commit `7fb530a`, and the local ROIs digest is now `876adf7626a1`
