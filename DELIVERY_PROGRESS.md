...
NEXT_ACTION: the route B v15 staged admission is now bound to the current code - 14 arrivals in 14 attempts under contract `ba46e2b2`, the death-confirmation policy live and the local ROIs `876adf7626a1` - so the next work is not another pass-binding attempt but the two named limits: the cold-start limit, which needs declared detour planning (drafted at `docs/ROUTE_B_DETOUR_DRAFT.md`, not implemented), and the mask's unmeasured isolated contribution, which a controlled traversability probe would settle. Keep the start-position caveat in front of anything downstream, now with the sharper statement that the pass needs a 1.5 px placement; do not re-run the admission, and do not use the recorded failure batches as substitutes
