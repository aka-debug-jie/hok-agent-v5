...
NEXT_ACTION: measure the mask's isolated contribution with a declared bounded traverse, which is the second open limit and the one the detour result sharpens: the frozen grid stores only a scalar projection per (cell, bearing), so at cell 14:12 it removes north on a 0.0795 px-per-100 ms projection while the measured bounded displacement is a 1.70 px west slide, and the mask's fallback to north-east then moves the hero east. A declared traverse of the thin cells (15:13 and 16:13 are near-empty) at the Router's own bounded press tier would give same-cell, same-tier rates from a declared stimulus rather than from incidental visitation, which is what `tier_restricted_rates` needs to say what the mask removes and what it keeps. Do not re-run the detour, do not re-run the admission, and do not use the failed batches as substitutes
...
CHANGED_FILES: AGENTS.md and BOUNDARIES.md (owner-authorized R1 scope extension), the R1 panel-gating contract and policy module, the read-only panel capture, the store-runner termination fixes, store contract v2, the route B contracts v1-v15, the declared progress guard, the region-safety filter, the declared deceleration table, the single-arbiter path planner, the declared bounded unknown-position recovery with its waypoint band, the declared approach press-release, direct-bearing and commitment rules, the measured traversability module and its frozen grid with its pinned source list, the traversability build and check commands and their make targets, the route B contract index, the controlled-response probe contracts, their analyser and the probe analysis mode, the probe and probe-analysis make targets, the read-only cue-position command, the death banner extent declaration and its loader check, the tier-restricted rate check, the corrected death banner box, the declared reposition contracts and their make target, `_retrace_direction`, `_toward_target_direction`, `_unknown_recovery_step`, `_mask_joystick_bearing`, `_commit_approach`, `_no_advance_detected`, the Router recovery path, the cross-episode pointer adoption, the declared reposition contracts `movement_goal_navigation_reposition_v3.json` and `v4.json` used to place the hero back in the recorded passing cluster for the rebind, the declared death-confirmation policy (`confirmation_steps`, `stationary_window_steps`, `maximum_travel_pixels`) in the observation ROIs with `_death_confirmed`, its loader check and its episode counters `death_banner_steps` and `death_confirmed_steps`, the declared bounded grid detour with `plan_grid_detour`, `validate_detour`, the `detour` runtime phase machine, its `detour_exhausted` outcome and its episode counters, the route B v18 and v19 contracts, the `DETOUR_FAILURE` error-class terminal reason in the store vocabulary, the declared reposition contracts `movement_goal_navigation_reposition_v5.json` and its cold-start placement, and the localisation, recovery, approach, guard and mask regressions in `tests/test_mobile_navigation_store.py`,
...
NEW_BYTES: 1,702,167,250 used by the active-probe and goal-navigation lineages to date; the 268,435,456 question cap is exceeded and the owner has stated there is no budget limit (this total adds the 24,524,501 of the detour attempt - the 3,415,769 and 2,185,474 placements and the 2,185,474, 2,185,474 and 14,552,890 detour batches - to the previous 1,677,642,749, which added the 120,626,453 of the bound rebind - the 3,415,769 tight placement and the 8,478,802, 25,963,286 and 82,768,596 stage-1, stage-3 and stage-10 batches - to the 1,557,016,296 before that, which added the 14,424,993 of the fresh `route-b-rebind2-stage1` run to the 1,542,591,303 before that, which added the 3,634,071 of the death-box negative check to the 1,538,957,232 before that, which added the 47,506,477 of the 2026-09-22 rebind to the 1,491,450,755 before that, which itself added the 222,454,107 of the five route B v15 device runs, the 4,248 of the probe re-analysis output, the 29,551,263 of the two probe sessions and their analysis, the 170,099 of the placement and pre-flight images, the 1,828,915 of the preserved death reference, the 85,962,294 of the two probe calibration runs and their analyses, the 2,270,045 of the two repositions and the cold-start batch, the 88,363,295 of the v16 and v17 cold-start attempts and their repositions, and the route B review's 4,177,652 completed form to the 1,060,808,197 before that)
...
NEXT_DECISION: the declared detour was tried and did not open the cold start, and its failure corrects the explanation rather than merely failing, so decide whether to fund the two sources it now names - a displacement-vector record per (cell, bearing) and evidence that a lateral route reaches a gap in the wall at 14:12 - or to leave the limit named and turn to the mask's isolated contribution with a declared bounded traverse; accept the pass as bound (14 of 14 from a 1.5 px placement, contract `ba46e2b2`, death policy live, ROIs `876adf7626a1`). Do not re-run the detour or the admission, do not start the life-state ROI, do not reuse the mobile banner geometry on native footage, and do not retune the frozen detector
```
