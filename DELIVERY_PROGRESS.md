...
NEXT_ACTION: fix the corrected death box before any further device run, because the rebind on 2026-09-22 showed the v15 pass does not reproduce under the current code: stage 1 passed 1 of 1 and stage 3 failed 1 of 3, with episode 1 stopped as `death_or_ended_screen` on a frame whose cue centroid equals the logged position and whose main and hud views show a live, un-greyed hero, so the enlarged `(683, 0, 937, 46)` box false-positives on a top-centre in-match announcement banner that clears the unchanged 2000-red and 80-white thresholds. Declare a discriminator for the death-replay prompt (its own layout, or cross-frame confirmation plus a hud life-state change) as a new versioned change with its own test before spending a session; stage 10 stays blocked and the recorded pass keeps its own commit `7fb530a` until then
...
NEXT_DECISION: first decide the death-box discriminator, because the rebind showed the enlarged box false-positives on an in-match announcement and no device run on this route is trustworthy until it is fixed or reverted; either declare a versioned discriminator for the death-replay prompt and test it offline against the corrected-box false positive and the owner death reference, or temporarily keep the pass scoped to commit `7fb530a`. Then, at the recorded scope, decide whether to accept route B (staged 1 then 3 then 10 from the measured starting positions) and, if the scope must widen, run the controlled traversability probe and a cold-start test rather than extrapolating; do not start the life-state ROI, do not reuse the mobile banner geometry on native footage, and do not retune the frozen detector
```
