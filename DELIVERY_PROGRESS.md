...
NEXT_ACTION: decide how to obtain a reproducible one-to-two-pixel start before any further attempt to bind the pass, because the fresh stage-1 rebind ran 97 steps to a budget timeout from `(55.1, 65.8)` with the death policy live and holding, and that start is 2.8 to 3.9 px from the recorded passing starts while the passing stage-1 run began 1.2 to 1.7 px away: the route is start-sensitive at the recorded three-pixel scale, and the declared reposition halts as soon as it is inside its 4.0 px arrival tolerance, so it cannot reproducibly deliver the start the pass needs. Options are a tighter declared placement that does not simply shrink a tolerance after a run, or accepting the scope as it stands. The rebind's failure and the pass's own commit `7fb530a` both still stand, and the local ROIs digest is now `876adf7626a1`
...
NEXT_DECISION: decide the placement question before any further pass-binding attempt, because the fresh stage-1 rebind timed out from a start 2.8 to 3.9 px off and the route is start-sensitive at the recorded three-pixel scale while the declared reposition cannot deliver a one-to-two-pixel start: either add a tighter declared placement, or accept the scope, or widen the evidence by capturing a frame of the offending non-death banner; then, at the recorded scope, decide whether to accept route B (staged 1 then 3 then 10 from the measured starting positions) and, if the scope must widen, run the controlled traversability probe and a cold-start test rather than extrapolating; do not start the life-state ROI, do not reuse the mobile banner geometry on native footage, and do not retune the frozen detector
```
