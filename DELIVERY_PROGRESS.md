...
STATUS: RUNNING
NEXT_ACTION: collect the read-only R1 panel samples (5 Hz, at least 1,200, split by episode, no device input), then train and evaluate the one-step-ahead candidate against the frozen myopic rule under the pre-declared bar
...
NEXT_DECISION: after the R1 comparison, either record the candidate as a gain and freeze its artifact, or retain the frozen rule and record R1 as no-gain; R1 may not claim independent feedback verification either way
```
