...
NEXT_ACTION: re-run route B under the region filter as soon as the device is awake with the game in the foreground, then apply the pre-declared stopping rule unchanged (if route B still arrives in none, stop and record the architecture limit rather than adding further offsets); the filter, the progress guard and the termination fixes are implemented and make check is green
...
CHANGED_FILES: AGENTS.md and BOUNDARIES.md (owner-authorized R1 scope extension), the R1 panel-gating contract and policy module, the read-only panel capture, the store-runner termination fixes, store contract v2, the route B generalisation contracts v1/v2/v3, the declared progress guard and the region-safety filter,
