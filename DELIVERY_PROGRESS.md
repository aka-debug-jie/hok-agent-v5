...
NEXT_DECISION: freeze the delivered scope or fund a different kind of source for the cold start, since three declared escape mechanisms have been tried on hardware and none gets through and the obstacle is the acceptance signal rather than the hold. The three candidate sources are a same-session placement-and-run so the start is exact, a corridor measured over its full length, and an acceptance signal that can actually be confirmed; anything else would be another rule on the same axis. Also pending and independent: a captured frame of the offending non-death banner, to replace the three counted rejections with a picture. The bound pass, the mask's measured contribution and the corridor span are unchanged. Do not re-run the persistence, the detour, the traverse or the admission; do not start the life-state ROI; do not reuse the mobile banner geometry on native footage; and do not retune the frozen detector
```
