...
NEXT_ACTION: fill the bounded-tier coverage gap with a declared bounded traverse from a placed position, because the mask's contribution is now measured from the recorded data over forty-seven cells (thirty-eight decisions, five removals, two of which are the largest-magnitude motion in their own cell) but the thin cells stay empty, since `15:13` has no bounded samples and `16:13` has four. A declared traverse of those two cells at the Router's own bounded press tier is the only remaining piece and the one measurement the recorded data cannot supply, and it also produces the displacement-vector record the detour named as missing: the frozen grid stores only a scalar projection per `(cell, bearing)`, so at `14:12` it removes north on a `0.0795` projection while the measured bounded displacement is a `1.70` px west slide. Do not re-run the detour, do not re-run the admission, and do not use the failed batches as substitutes
