...
NEXT_ACTION: make the declared progress-guard escape region-aware (skip or re-rank an offset whose rotated bearing would leave the free-movement region) and re-run route B under unchanged gates; the guard already fires and escapes live (13 events, 36 escape steps) but its rotated bearings drove the hero out of the region in all three episodes, so route B still arrives in none and the validated scope remains the two-waypoint diagonal
...
CHANGED_FILES: AGENTS.md and BOUNDARIES.md (owner-authorized R1 scope extension), the R1 panel-gating contract and policy module, the read-only panel capture, the store-runner termination fixes, store contract v2, the route B generalisation contracts v1/v2 and the declared progress guard,
...
NEW_BYTES: 773,898,582 used by the active-probe and goal-navigation lineages to date (531,241,179 probe plus 65,178 goal-navigation plus 48,977,309 for the a2/a3 goal-navigation runs plus 2,936,087 for the single store-bound run plus 13,549,043 for the first two store-bound batches plus 10,363,263 for the R0 evidence batch and its feedback audit plus 15,376 for the four response audits plus 6,409 for the two panel audits plus 11,115,714 for the two read-only panel capture pilots plus 65,853,403 for the three read-only termination-defect validation runs plus 11,735,702 for the two bounded-recovery validation runs plus 28,607,477 for the first route B batch plus 31,049,060 for the first progress-guard batch plus 18,180,327 for the second progress-guard batch); the 268,435,456 question cap is exceeded and the owner has stated there is no budget limit
...
NEXT_DECISION: either fund the region-aware escape and the route B re-run, or record route B as the generalisation limit and keep the chain frozen on the two-waypoint diagonal it was validated on; R still needs an external independent reference
```
