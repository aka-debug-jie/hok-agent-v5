...
- Operation Movement Teacher v1 may use the Mobile Operation Base two-pointer transport only in
  ...
  The marksman opener may use only the frozen north-east/east blue route or its center-mirrored
  south-west/west red route before formal collection. It is summary-only, cannot run without a
  verified side, and cannot be relabeled as minimap-teacher supervision.
...
- Stage 6A may run a separately authorized, explicit-serial, V4L2-only Global Agent Shadow after
  ...
  layout coordinates, purchase dispatch, and online learning are unavailable. Any guard or capture
  failure stops the session with zero input.
