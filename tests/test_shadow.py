...
def test_shadow_local_video_is_read_only_and_always_abstains(
    ...
    _video(video)
    save_model(model, PixelActor(), PixelArena().config, 0)
    ...
    assert {path.name for path in output.iterdir()} == {"predictions.jsonl", "summary.json"}
    ...
    assert summary["abstain_count"] == len(rows)
    assert summary["advisory_count"] == 0
    assert summary["real_domain_validated"] is False
    assert summary == json.loads((output / "summary.json").read_text())
    assert str(video) not in (output / "predictions.jsonl").read_text()
    after = video.stat()
...
def test_shadow_refuses_existing_output(tmp_path: Path) -> None:
    ...
    _video(video)
    save_model(model, PixelActor(), PixelArena().config, 1)
    output.mkdir()
...
def test_shadow_rejects_non_frozen_model_and_mislabeled_playlist(tmp_path: Path) -> None:
    ...
    model = tmp_path / "model.safetensors"
    _video(video)
    save_model(model, PixelActor(), PixelArena().config, 2)
