...
def record_episode(
    ...
    with PixelArenaService() as service:
        ...
        documents = [header]
        moves = attacks = structure_damage = 0
            ...
            documents.append(
                {
                    ...
                    "blue_action": blue_action.to_dict(),
                    "red_action": red_action.to_dict(),
                    "observation_hash": _state_hash(response),
                    "events": events,
                    "terminal": response["terminal"],
                    "outcome": response["outcome"],
                }
            )
        blue_observation = cast(
            dict[str, object], cast(dict[str, object], response["observation"])["blue"]
        )
        _write_jsonl(path, documents)
        return {
            "path": str(path),
            "ticks": blue_observation["tick"],
            "outcome": response["outcome"],
            "terminal": response["terminal"],
            "moves": moves,
            "attacks": attacks,
...
def _read_jsonl(path: Path) -> list[dict[str, object]]:
    ...
    for line in path.read_text().splitlines():
        ...
        if not isinstance(document, dict):
            raise ReplayError("trace rows must be objects")
...
def _valid_action_document(value: object) -> bool:
    ...
    return all(type(value[key]) is str for key in string_keys) and type(value["auxiliary"]) is int
...
def _validate_trace_types(header: dict[str, object], rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        if type(row["tick"]) is not int:
            raise ReplayError("invalid transition types")
...
def verify_trace(path: Path) -> dict[str, object]:
    ...
    if set(header) != HEADER_KEYS:
        raise ReplayError("invalid trace header fields")
    required_header = {
        "kind": "pixelarena_public_trace_v1",
        "claim_scope": "pixelarena_engineering",
        "hok_capability_claim": False,
    ...
    for row in rows:
        if set(row) != ROW_KEYS:
            raise ReplayError("invalid transition fields")
    ...
    with PixelArenaService() as service:
        ...
        for key, actual in (
            ...
            if header.get(key) != actual:
                raise ReplayError(f"header mismatch: {key}")
        ...
        for expected_tick, row in enumerate(rows, start=1):
            blue_action = FactorizedAction.from_dict(
                cast(dict[str, object], row["blue_action"])
            )
        ...
        if not bool(response["terminal"]):
            raise ReplayError("trace does not end at terminal")
        return {
            "verified": True,
            "ticks": len(rows),
            "outcome": response["outcome"],
...
def accept_minimal_v1(seed: int, output_dir: Path | None = None) -> dict[str, object]:
    ...
    for name, blue, red, expected_outcome in definitions:
        ...
        recorded = record_episode(path, blue, red, seed)
    ...
    if not tamper_rejected:
        raise ReplayError("tampered trace was accepted")
    return {
        "acceptance": "PASSED",
        "claim_scope": "pixelarena_engineering",
        "hok_capability_claim": False,
