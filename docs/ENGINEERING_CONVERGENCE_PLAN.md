...
首次 C 运行前落实以下 dev 工程门；结果出来后不降低门槛：
...
在任何第二次训练前，下一步仅允许另建评估合同：保持24个既有dev场景和21/24、碰撞、折返
门槛不变，把随机对照改为固定有限步预算或成对效率指标，并在看到第二候选前冻结。不得使用
holdout选门。训练数据中E/W/STOP窗口明显多于N/S等方向，且未包含学生偏离后的恢复轨迹；
它们是下一次数据修正候选，不是已证明原因，不同时改采样和模型。
...
下一步只有 C：生成 64/24 条动作驱动 train/dev 轨迹并训练一次任务专用 Movement 初始候选，
随后独立运行 24 局 dev。保持 simulator-only，不扫描视频、不连接手机，也不启动 E1 终局研究。
